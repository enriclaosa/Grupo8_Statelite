...
def EnviarValor(): # Botón enviar
    valor = ValorEntry.get()
    global limite_alarma

    if accion_actual == "periodo": # Envia el nuevo periodo de envio de datos 
        mensaje = f"1 {valor}\n"
        mySerial.write(mensaje.encode('utf-8'))
    elif accion_actual == "orientacion": # Envia la nueva orientación del servomotor
        try:
            valor_int = int(valor)
            ...
            return
    elif accion_actual == "ValorTempMax": # Se ajusta un nuevo valor máximo de temperatura mediana
        try:
            limite_alarma = float(valor)
        except ValueError:
            MensajeVar.set("Introduce un número válido")
            return
    elif accion_actual == "observacion": # Se registra una observacion introducida por el usuario
...
def leer_datos_serial():
    ...
    while mySerial.in_waiting > 0:
        try:
            ...
            # MEDIA (código 3)(solo si se está calculando en el satélite)
